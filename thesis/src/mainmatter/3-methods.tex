...
\subsection{Controlling for corpus size}
\label{sec:3.4.2}

As discussed in \secref*{sec:1.3}, some researchers suggest that flexibility should increase as a function of corpus size. The intuition behind this claim is that the larger the corpus, the more likely there are to be flexible uses of any given lexical item. To test this claim, I took differently-sized samples of each corpus, and calculated the average flexibility for all lexical items in each sample.

Because of the drastically different sizes of the two corpora, I used slightly different techniques for comparing sample sizes in English and Nuuchahnulth. For English, I calculated the average flexibility for only the words in the 100-item sample for corpus sizes in multiples of 10,000. That is, I calculated the average flexibility of the 100 items for a corpus of 10,000 tokens, and then the average flexibility of those items in a corpus of 20,000 tokens, then 30,000, and so on. This resulted in $\sim$150 measurements. For Nuuchahnulth, I calculated the average flexibility of corpus sizes in multiples of 1,000 instead, resulting in 8 measurements. Additionally, for Nuuchahnulth I included every lexical token, not just the stems in the 100-item sample.

After gathering this data on average lexical flexibility for different corpus sizes, I applied a generalized additive model \TODO{Visualize the data first and see whether a GAM is really necessary, or whether a simple linear model will do the trick.} (GAM) to examine the relationship between the two variables. I present the results of this model in \secref*{sec:4.4}.

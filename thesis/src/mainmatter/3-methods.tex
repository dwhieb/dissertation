...
\noindent It is worth mentioning that annotating the Nuuchahnulth corpus for discourse function would not have been practical without the detailed descriptive work of Toshihide Nakayama. The creation of text collections is often underappreciated as a worthwhile academic endeavor, but this is the only way we gain new corpora of naturalistic discourse for minority languages. The empirical and theoretical findings of this project would not have been possible without this important work.
...
As discussed in \secref*{sec:1.3}, some researchers suggest that flexibility should increase as a function of corpus size. The intuition behind this claim is that the larger the corpus, the more likely there are to be flexible uses of any given lexical item. This is the basis for \ref{R2}, \enquote{Is there a correlation between degree of lexical flexibility and size of the corpus?}. To test this claim, I took differently-sized samples of each corpus, calculated the flexibility of the lexical items in each sample, and then examined whether that flexibility changed as a function of corpus size.

Because of the drastically different sizes of the two corpora, I used slightly different techniques for comparing sample sizes in English and Nuuchahnulth. For English, I calculated the flexibility of words in the 100-item sample for corpus sizes in multiples of 10,000. That is, I calculated the flexibility of the 100 items for a corpus of 10,000 tokens, and then the flexibility of those same items in a corpus of 20,000 tokens, then 30,000, and so on. For Nuuchahnulth, I calculated the flexibility of corpus sizes in multiples of 1,000 instead. Additionally, for Nuuchahnulth I included every lexical token, not just the stems in the 100-item sample.

\TODO{Adjust this paragraph based on what you wind up doing. You might for example take the average flexibility ratings, but show the error bars on the graph.}

After gathering the flexibility data for different corpus sizes, I applied a generalized additive model (GAM) to examine the relationship between the two variables. I present the results of this model in \secref*{sec:4.4}.

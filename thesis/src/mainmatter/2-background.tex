...
It should be noted at the outset that many of these findings cannot be straightforwardly reinterpreted in the typological-constructional approach adopted here. In particular, much previous research relies heavily on large, language-specific categories, which I problematize in \secref*{sec:2.2.3} above. I discuss some of the other criticisms that have been leveled against these approaches in \secref*{sec:2.3.3}. My use of terms like \txn{Noun}, \txn{Verb}, and \txn{Adjective} in this section should therefore not be taken as an endorsement of large, language-particular word classes, but instead as arising from a desire to accurately represent the work of other researchers.
...
\begin{description}
  \item[lexical flexibility] The use of a lexical item (root, stem, or inflected word) in more than one discourse function (reference, predication, or modification) with no overt morphological indication of that function.
\end{description}
...
\begin{description}
  \item[functional expansion] A diachronic expansion in the use of a lexical item (root, stem, or inflected word) into a new discourse function (reference, predication, or modification) with no additional morphological indicator of that function.
\end{description}

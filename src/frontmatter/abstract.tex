\section*{Abstract}
\addcontentsline{toc}{section}{Abstract}

\begin{center}

  \doctitle

  by

  \theauthor

\end{center}

\todo[
  caption={add Abstract},
  inline,
  size=normalsize
]{The abstract should include 1) a brief statement of the problem; 2) a description of the methods and procedures used to gather data or study the problem; 3) a condensed summary of the findings. The abstract should be double-spaced. \citetitle{Bach1968} The recommended length is 1--2 pages \cite{Bach1968,Boas1911}. \index{Chitimacha}}

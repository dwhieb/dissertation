...
To accomplish this, I wrote an R script called `clean_data.R`, located at `scripts/data/clean_data.R`. Running this script will generate a cleaned version of each CoNLL in the data directory in this repository. To run this script on a data directory located elsewhere, simply change the `data_dir` variable at the top of the file to point to the directory where your data are located.

### Stage 6: Quantitative Analysis

Once the data are cleaned, they are ready for statistical analysis. I chose to calculate statistics at three different linguistic levels—wordform, lexeme, and archlexeme. The `scripts/stats` directory contains one script for each of these levels:

  - `generateWordforms.r`: Generates a tab-delimited file called `wordforms.tsv` containing each unique wordform in the corpus and its raw frequency

  - `generateLexemes.r`: Generates a tab-delimited file called `lexemes.tsv` containing each lexeme in the corpus and its raw frequency

  - `generateArchlexemes.r`: Generates a tab-delimited file called `archlexemes.tsv` containing each archlexeme in the corpus and its raw frequency
...
By default, each script processes the English data in `data/English/data`, and generates its associated file in `data/English/stats`. To change the input directory, change the `data_dir` variable at the top of each script. To change the path where the file containing statistical data is generated, change the value of the variable `output_path` at the top of each script.

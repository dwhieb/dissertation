...
create_graphic <- function(data) {
  ...
  data$log_rel_freq <- log2(data$rel_freq)
  ...
  model_Eng <- lm(Eng$log_rel_freq ~ Eng$flexibility)
  model_Nuu <- lm(Nuu$log_rel_freq ~ Nuu$flexibility)
  ...
  boxplot <- ggplot(data, aes(
    ...
    xlab("log relative frequency (per 1,000 words)") +
    geom_boxplot(
      na.rm       = TRUE,
      notch       = TRUE,
      show.legend = FALSE,
      width       = 1 # possibly 0.5
    ) +
    facet_grid(cols = vars(language))
...
ggsave(
  "stats/figures/frequency_vs_flexibility/100.png",
  graphic_100,
